...
#######################################################################################################################
    ...
    async def cmd_help(self, command=None):
        ...
        if command:
            ...
            #helpmsg += "```\nSong request are now open!\n https://docs.google.com/forms/d/1aVNVK23ScWntIxdrkNyo0MM4YW2ptWxt_YQmtOM_mKU \n**MewBot | Version 5.3.0**"
            helpmsg += "```\nSong request are now closed!\n**MewBot | Version 5.3.0**"
    ...
    async def cmd_uptime(self, channel):
        ...
        {command_prefix}uptime
              Show how long has the bot been running
              NOTE: This Function is currently still in alpha dev and the uptime is all in seconds and miliseconds.
              A better version of uptime will be deploy in the future.
        """
        #return Response("Coming Soon! :thumbsup:")
        currentuptime = uptime - time.time()
        return Response(currentuptime)
	

 #   async def cmd_addsong(self, player, channel, server, message): 
#
 #        """
  #       Usage:
   #         {command_prefix}addsong current song
    #        {command_prefix}addsong song_url
#
...
#
#
    async def cmd_reset(self, author, channel):
